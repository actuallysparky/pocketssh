...
build_dir="${POCKETSSH_BUILD_DIR:-${esp_root}/_local/build/pocketssh/tpager}"
package_dir="${POCKETSSH_PACKAGE_DIR:-${esp_root}/_local/packages/pocketssh}"
src_bin="${build_dir}/PocketSSH.bin"
dst_bin="${package_dir}/PocketSSH-TPager.bin"

if [[ ! -f "${src_bin}" ]]; then
  echo "Missing source binary: ${src_bin}" >&2
  echo "Run idf.py -B ${build_dir} -DTPAGER_TARGET=ON -DTPAGER_DIAG=OFF build first." >&2
  exit 2
fi
