...
static esp_err_t panel_st7796_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end,
...
static esp_err_t panel_st7796_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]) {st7796->madctl_val}, 1), TAG,
                        "send command failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD, (uint8_t[]) {st7796->colmod_val}, 1), TAG,
                        "send command failed");

    int cmd = 0;
    while (vendor_specific_init[cmd].data_bytes != 0xFF) {
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(io, vendor_specific_init[cmd].cmd, vendor_specific_init[cmd].data,
                                          ...
                                          const void *color_data)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_color(io, LCD_CMD_RAMWR, color_data, len), TAG, "send RAMWR failed");
